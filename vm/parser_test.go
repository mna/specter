...
var (
	assertOps = map[string][]opcode{
		...
		// No need to test loop2 and loop3, exact same files as loop.vm, only the iterations are changed.
		"loop.vm": []opcode{
			_OP_INC,
			_OP_CMP,
			_OP_PRN,
	...
	assertLabels = map[string]map[string]int32{
		...
		"loop.vm": map[string]int32{
			"loop": 0,
		},
		"nop.vm": map[string]int32{
			"loop": 1,
		},
		"primes.vm": map[string]int32{
			"start":       0,
			"checkPrime":  1,
			"checkFactor": 2,
			"primeFound":  10,
		...
		"stack_bench.vm": map[string]int32{
			"start": 0,
			"loop0": 2,
			"loop1": 7,
		},
